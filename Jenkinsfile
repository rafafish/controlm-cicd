    ...
    stages {
        stage('Build') {
            environment {
                ...
                ENDPOINT = 'https://srvcontrolm02:8443/automation-api'
            }
            steps {
                sh '''
                ...
                # Build
                curl -k -s -H "Authorization: Bearer $token" -X POST -F "definitionsFile=@ctmjobs/conn-profiles.json" "$ENDPOINT/build"
        ...
        stage('Deploy') {
            ...
            steps {
                ...
                curl -k -s -H "Authorization: Bearer $token" -X POST -F "definitionsFile=@ctmjobs/conn-profiles.json" "$ENDPOINT/deploy"
